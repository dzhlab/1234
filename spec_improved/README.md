...
### 11. TODO.md (500 строк)

**Описание:** Список задач на будущее

**Содержание:**

#### Приоритет 1 (MUST) - 4 задачи:
1. Исправление противоречий
2. Добавление недостающих разделов в главный документ
3. Создание руководства по развёртыванию
4. Создание тест-кейсов для QA

#### Приоритет 2 (SHOULD) - 6 задач:
5. Добавление acceptance criteria
6. Создание карты экранов
7. Алгоритм расчёта в псевдокоде
8. Примеры JSON для API
9. Docker Compose
10. Руководства пользователя

#### Приоритет 3 (COULD) - 5 задач:
11. Перевод на английский
12. FAQ
13. Скриншоты интерфейсов
14. Видео-инструкции
15. Интерактивные туториалы

#### Приоритет 4 (RESEARCH) - 3 задачи:
16. Интеграция с видеосистемой
17. Мобильное приложение
18. AI для автоматической оценки
...
1. **Backend:** `API_SPECIFICATION.md` → `DATABASE_SCHEMA.md` → `SCORING_ALGORITHM.md`
2. **Frontend:** `SITEMAP.md` → `API_SPECIFICATION.md` → `USER_STORIES.md`
3. **DevOps:** `NON_FUNCTIONAL_REQUIREMENTS.md` → `DIAGRAMS.md` (раздел развертывания)

### Для QA:

1. **Тестирование:** `USER_STORIES.md` (acceptance criteria) → `SCORING_ALGORITHM.md` (для тестов логики)
2. **API тесты:** `API_SPECIFICATION.md`
